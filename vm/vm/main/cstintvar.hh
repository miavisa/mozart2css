...
CstIntVar::CstIntVar(VM vm, RichNode min, RichNode max) 
  ...
  nativeint mn = getArgument<nativeint>(vm,min, "integer");
  nativeint mx = getArgument<nativeint>(vm,max, "integer");
  if (!validAsElement(mn))
    raiseTypeError(vm, "IntVarLike", mn);
  if (!validAsElement(mx))
    raiseTypeError(vm, "IntVarLike", mx);
